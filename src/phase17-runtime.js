(function defineEversteadPhaseSeventeenRuntime(root){
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='everstead-phase17-styles';style.textContent=`
      .phase17-wayfarer{display:grid;grid-template-columns:84px minmax(0,1fr);gap:11px;align-items:center;margin:10px 0;border-color:#66e1d255;cursor:pointer}.phase17-wayfarer>div:last-child{min-width:0}.phase17-wayfarer:focus-visible{outline:3px solid var(--gold);outline-offset:2px}
  ...
  campaignView=function(){const html=campaignViewBeforePhaseSeventeen(),withoutPredecessor=html.replace(/<section class="card player-rank player-profile-trigger"[^>]*>[\s\S]*?<\/section>/,''),mode=motionState(),marker=`<section class="card phase17-wayfarer" role="button" tabindex="0" data-player-profile data-phase17-campaign-identity-card="player.wayfarer" data-phase17-player-character="player.wayfarer" data-phase17-player-mode="${campaignPresentationDefault}" data-phase17-player-asset-approval="approved-original-source" data-phase17-player-background="retained" data-phase17-player-transparency-claim="none" data-player-roster-member="false" data-player-combat-power="none" data-motion-state="${mode}" aria-label="Open The Wayfarer full-art Player profile"><div class="phase17-wayfarer-mark" data-art-treatment="framed-background-static" data-image-state="loaded"><img src="assets/player/wayfarer-profile-full.png" width="1024" height="1536" alt="The Wayfarer framed against Everstead" decoding="async" onerror="this.parentElement.dataset.imageState='fallback';this.parentElement.dataset.fallbackMode='${campaignPresentationFallback}';this.hidden=true"></div><div><div class="eyebrow">Player Character · Rank ${currentState().player.rank}</div><h3>The Wayfarer</h3><p>A static framed view keeps the original background visible. He leads the journey without entering Fellow, Family, or Companion rosters or combat Power.</p></div></section>`;return marker+withoutPredecessor};
