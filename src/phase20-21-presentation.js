(function installEversteadPhaseTwentyTwentyOnePresentation(root){
  ...
  function install(adapters){
    ...
    function requirementHtml(facilityId,record){
      ...
      return`<div class="es-requirement-row" data-phase20-21-requirement="${esc(requirement.stockId)}"><div><span>Stock requirement</span><b>${esc(requirement.label)}</b></div><div><span>${reserved?'Reserved':'Available'}</span><b>${reserved||available} / ${requirement.quantity}</b></div><p>${reserved?'This exact reservation persists through close and Resume.':'Beginning reserves the exact amount. Cancel before Commit restores it.'}</p></div>`;
    }
    ...
    function rewardHtml(record){
      ...
      return`<section class="es-reward-preview" data-phase20-21-claim-ready><div><div class="eyebrow">Manual reward claim</div><h3>${record.outcome.globalGold.toLocaleString()} Gold waiting</h3></div><p>No reward or local progress has been applied yet. Claim commits both exactly once.</p></section>`;
    }
    ...
    function sheetHtml(){
      ...
      return`<section class="phase20-21-sheet" tabindex="-1" data-phase20-21-sheet data-facility-id="${esc(ui.facilityId)}" data-phase20-21-state="${esc(facility.state)}" data-phase20-21-lifecycle-status="${esc(lifecycle)}" aria-labelledby="everstead-modal-title"><header class="es-feature-masthead"><div><div class="eyebrow">${item.phase===20?'Original Village facility':'Village expansion'} · ${esc(item.anchor.replaceAll('-',' '))}</div><h2 id="everstead-modal-title">${esc(item.name)}</h2><p>${esc(item.activityName)} · guided by ${esc(humanActor(item.presenterActorId))}</p></div><button class="close" data-phase20-21-close aria-label="Close ${esc(item.name)}">×</button></header><div class="es-stat-ribbon"><div><span>Board state</span><b>${esc(statusCopy(facility.state))}</b></div><div><span>Waiting</span><b>${facility.records.length}</b></div><div><span>Claims</span><b>${facility.claimedCount}</b></div><div><span>Local progress</span><b>${facility.localProgress}</b></div></div>${contextualTutorialHtml()}${selected?recordHtml(ui.facilityId,selected):`<section class="phase20-21-empty" data-phase20-21-empty><div class="eyebrow">Nothing waiting</div><h3>${esc(item.activityName)} will bank here</h3><p>Passive Village production continues. Active opportunities do not expire and will appear when their approved cadence creates one.</p></section>`}${opportunityRowsHtml(facility,selected)}${lastReceipt?`<section class="es-progress-record" tabindex="-1" data-phase20-21-receipt-summary><span>Latest exact-once receipt</span><b>${esc(lastReceipt)}</b></section>`:''}<footer class="phase20-21-sheet-footer">${passiveBuilding?`<button class="btn" data-phase20-21-passive="${esc(passiveBuilding)}">MANAGE PASSIVE PRODUCTION</button>`:''}<button class="btn" data-phase20-21-open-tutorial-log>TUTORIAL LOG · 79 + 19</button></footer><p class="phase20-21-policy-note">Passive play keeps Everstead progressing. Active play accelerates the Village. Rewards remain manual claims and opportunities never expire.</p></section>`;
    }
