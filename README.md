...
`main` is a runnable schema-12 release-candidate build through Phase 11H. The locked Oath, Building, Family, Fellow, Companion, Campaign, Tower, Expedition, Relic, Player Rank, offline-reward, and activated economy systems are implemented behind the preserved mobile shell. Phase 11F externalized the portrait library and expanded the roster. Phase 11G makes that roster playable: six Fellows start, twelve join deterministically across Player Ranks 2–5, Campaign rewards rotate across every Rank group, and established saves receive a bounded one-time EXP floor with explicit activation provenance. Phase 11H gives the Village its own transparent dialogue cutouts while preserving the full-background art on character sheets.

The Locked Core Design controls product behavior; the roadmap controls migration order. See the [Phase 11H dialogue-cutout contract](docs/PHASE_11H_DIALOGUE_CUTOUT_CONTRACT.md), [Phase 11H result](docs/PHASE_11H_RESULT.md), [Phase 11G progression contract](docs/PHASE_11G_PROGRESSION_CONTRACT.md), [Phase 11G result](docs/PHASE_11G_RESULT.md), [Prosperity/HQ decision record](docs/PROSPERITY_HQ_DECISION.md), [roster catch-up decision record](docs/ROSTER_CATCH_UP_DECISION.md), [future-schema recovery policy](docs/RECOVERY_SCHEMA_POLICY.md), and [source structure map](docs/PHASE_11E_STRUCTURE_MAP.md).
