...
Historical baseline Offline Gold coverage includes a table of fixed expected outcomes for 0 and 1 milliseconds, the old 60,000/60,001 millisecond modal boundary, exactly two hours, an immediate second claim, 24 hours minus 1 millisecond, exactly 24 hours, 24 hours plus 1 millisecond, a missing timestamp, and same-day/next-day rollover. Phase 11A supersedes only the current player-facing modal threshold with a five-minute boundary; settlement arithmetic remains unchanged. A separate scenario clicks the offline summary's special claim action and verifies whole-Gold transfer, pending reset, exact memory-storage persistence, modal closure, deterministic re-boot behavior, and a zero-value immediate second claim. Expected numbers are stored as constants in scenario data; the harness does not reuse production economy formulas.
