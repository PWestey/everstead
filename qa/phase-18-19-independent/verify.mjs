...
const EXPECTED=['README.md','checksums.sha256','fixtures/contract-fixtures.json','fixtures/inherited-hashes.json','fixtures/policy-approval-fixtures.json','index.html','realm.html','realm.js','runner.js','verify.mjs'];
const rows=[];
...
const policyFixtures=json('qa/phase-18-19-independent/fixtures/policy-approval-fixtures.json');
const candidatePolicy=json('design/phase-18-19-policy-candidate/policy-candidate.json');
const candidateTable=json('design/phase-18-19-policy-candidate/fixed-reward-table.json');
const candidateSimulation=json('design/phase-18-19-policy-candidate/simulation-results.json');
const candidateTutorials=json('design/phase-18-19-policy-candidate/tutorial-timing.json');
const candidateCast=json('design/phase-18-19-policy-candidate/cast-schedule.json');
const candidateFixtures=json('design/phase-18-19-policy-candidate/fixtures.json');
const approvedPolicy=json('design/phase-18-19-policy-approved/policy-approved.json');
const approvalRecord=json('design/phase-18-19-policy-approved/approval-record.json');
...
const nullPointers=(value,path='')=>{
  if(value===null)return[path];
  if(Array.isArray(value))return value.flatMap((child,index)=>nullPointers(child,`${path}/${index}`));
  if(value&&typeof value==='object')return Object.entries(value).flatMap(([key,child])=>nullPointers(child,`${path}/${key}`));
  return[];
};
const atPointer=(value,path)=>path.replace(/^\//,'').split('/').reduce((current,token)=>Array.isArray(current)?current[Number(token)]:current[token],value);
...
record('package-checksums',checksumRows.length===23&&checksumFailures.length===0,{count:checksumRows.length,failures:checksumFailures});
...
record('contract-scope-is-private-and-sequentially-gated',['private-release activities','does not modify accepted design','retargeted sequentially onto exact accepted Phase 17 integration tip','released Phase 15 pending-offer/V2 receipt/archive authority','public release false'].every(value=>contract.includes(value)));
...
record('contract-offline-concurrency-recovery',['never expire','two-client','fresh, migrated, offline, recovery, import, future, corrupt','512 recent receipts','folds 128','24 elapsed-hour offline cap','passiveBaseline(capturedAt)','excludes volatile balances/timestamps and raw `boostDay`'].every(value=>contract.includes(value)));
record('contract-five-realms-actual-dom',['320×568','390×844','1024×768','130-percent copy','reduced motion','actual DOM nodes/styles','44×44','focus entry and return','Escape','Reduced-motion coverage is intentionally non-tautological','data-everstead-reduced-motion="reduce"','monkey-patched `matchMedia().matches` value by itself is insufficient'].every(value=>contract.includes(value)));
...
record('fixture-distinct-facility-identities',fixtures.facilities.apothecary.facilityId==='facility.apothecary'&&fixtures.facilities.schoolhouse.facilityId==='facility.schoolhouse'&&fixtures.facilities.apothecary.activityId!==fixtures.facilities.schoolhouse.activityId&&fixtures.facilities.apothecary.mapAnchor==='eastern-plaza-apothecary'&&fixtures.facilities.schoolhouse.mapAnchor==='eastern-plaza-schoolhouse');
...
record('fixture-three-finalizers-graduation-v2',fixtures.finalizers.length===3&&unique(fixtures.finalizers.map(item=>`${item.sourceId}:${item.domainClaimKind}`))&&fixtures.finalizers.every(item=>item.immutable===true&&item.callerSupplied===false)&&fixtures.schoolhouse.graduationOfferTemplate.endsWith('.v<definitionVersion>')&&fixtures.schoolhouse.graduationDomainClaimKind==='schoolhouse-graduation');
...
record('fixture-state-risk-matrices',Object.keys(fixtures.saveFixtures).length===24&&unique(Object.values(fixtures.saveFixtures))&&fixtures.saveFixtures.apothecaryIntroduction==='p18.qa.apothecary-introduction.v1'&&fixtures.saveFixtures.schoolhouseIntroduction==='p19.qa.schoolhouse-introduction.v1'&&fixtures.invalidMutationChecks.length===16&&unique(fixtures.invalidMutationChecks)&&fixtures.concurrencyKinds.length===8&&unique(fixtures.concurrencyKinds));
record('fixture-archive-offline-passive',same(fixtures.archivePolicy,{configId:'claim-archive.phase-15.v1',recentReceiptLimit:512,foldBatchSize:128,expectedRecentAfterNextClaim:385,expectedThroughAfterNextClaim:128})&&fixtures.offlineCapMs===86400000&&fixtures.originalPassiveFacilityIds.length===4&&fixtures.passiveSnapshotContract.capturedAt===fixtures.frozenNow&&fixtures.passiveSnapshotContract.buildingFields.includes('effectiveOathBoost')&&fixtures.passiveSnapshotContract.excludedVolatileFields.includes('boostDay')&&fixtures.passiveSnapshotContract.excludedVolatileFields.includes('goldBalance'));
record('fixture-five-realms',fixtures.viewports.length===5&&fixtures.viewports.some(item=>item.width===320&&item.height===568)&&fixtures.viewports.some(item=>item.width===390&&item.height===844)&&fixtures.viewports.some(item=>item.width===1024&&item.height===768)&&fixtures.viewports.some(item=>item.copyScale===1.3)&&fixtures.viewports.some(item=>item.reducedMotion)&&fixtures.reducedMotionContract.rootMarkerAttribute==='data-everstead-reduced-motion'&&fixtures.reducedMotionContract.javascriptMatchMediaAloneInsufficient===true);
record('fixture-thirteen-candidate-boundaries-registered',fixtures.requiredCandidateBoundaryIds.length===13&&unique(fixtures.requiredCandidateBoundaryIds));
record('fixture-approved-runtime-expects-no-boundary-failures',same(fixtures.expectedCandidateFailureIds,[]));
...
record('policy-fixture-candidate-only',policyFixtures.fixtureId==='phase-18-19-policy-approval-fixtures-v1'&&policyFixtures.status==='candidate-qa-only'&&policyFixtures.productionEnabled===false&&policyFixtures.releaseRule.includes('never satisfy the 13 absent production-runtime boundaries'));
record('policy-fixture-86-null-slots',same(policyFixtures.acceptedNullSlots,{apothecary:38,schoolhouse:48,total:86}));
const apothNulls=nullPointers(apothecary),schoolNulls=nullPointers(schoolhouse),resolvedApoth=candidatePolicy.phase18ApothecaryPolicy.resolvedDesign,resolvedSchool=candidatePolicy.phase19SchoolhousePolicy.resolvedDesign;
record('candidate-resolves-exact-86-design-nulls',apothNulls.length===38&&schoolNulls.length===48&&[...apothNulls.map(path=>atPointer(resolvedApoth,path)),...schoolNulls.map(path=>atPointer(resolvedSchool,path))].every(value=>value!==null));
record('candidate-does-not-unblock-accepted-design',candidatePolicy.status==='candidate-root-review-required'&&candidatePolicy.authoritative===false&&candidatePolicy.productionEnabled===false&&candidatePolicy.mechanicalEnablementAllowed===false&&apothecary.productionEnabled===false&&schoolhouse.productionEnabled===false);
record('approved-policy-is-exact-immutable-successor',approvedPolicy.status==='approved-private-release'&&approvedPolicy.authoritative===true&&approvedPolicy.productionEnabled===true&&approvedPolicy.privateReleaseOnly===true&&approvedPolicy.publicReleaseAllowed===false&&approvalRecord.sourceCandidateCommit==='513b2f0e4d9aa8498770e48ea3faf04c515f2aa9'&&approvalRecord.approvedValuesChanged===false&&approvalRecord.sourceCandidateHashes['policy-candidate.json']===sha(read('design/phase-18-19-policy-candidate/policy-candidate.json'))&&approvalRecord.sourceCandidateHashes['fixed-reward-table.json']===sha(read('design/phase-18-19-policy-candidate/fixed-reward-table.json'))&&approvalRecord.sourceCandidateHashes['tutorial-timing.json']===sha(read('design/phase-18-19-policy-candidate/tutorial-timing.json'))&&approvalRecord.sourceCandidateHashes['cast-schedule.json']===sha(read('design/phase-18-19-policy-candidate/cast-schedule.json')));
record('candidate-apothecary-values',resolvedApoth.facility.operational.intervalMs===policyFixtures.apothecary.intervalMs&&resolvedApoth.facility.operational.bankCapacity===policyFixtures.apothecary.bankCapacity&&resolvedApoth.facility.operational.unattendedTargetMs===policyFixtures.apothecary.unattendedTargetMs&&same(Object.values(candidatePolicy.phase18ApothecaryPolicy.masteryProgression.levelThresholds),policyFixtures.apothecary.masteryThresholds)&&same(resolvedApoth.regions.map(item=>item.selectionWeight),policyFixtures.apothecary.regionWeights)&&same(resolvedApoth.caseTemplates.map(item=>item.selectionWeight),policyFixtures.apothecary.caseWeights)&&same(resolvedApoth.namedPatients.map(item=>item.selectionWeight),policyFixtures.apothecary.namedPatientWeights));
record('candidate-schoolhouse-values',resolvedSchool.facility.operational.intervalMs===policyFixtures.schoolhouse.intervalMs&&resolvedSchool.facility.operational.bankCapacity===policyFixtures.schoolhouse.bankCapacity&&resolvedSchool.facility.operational.unattendedTargetMs===policyFixtures.schoolhouse.unattendedTargetMs&&candidatePolicy.phase19SchoolhousePolicy.decisionDepth.initialSeatCount===policyFixtures.schoolhouse.initialSeats&&candidatePolicy.phase19SchoolhousePolicy.decisionDepth.maximumSeatCount===policyFixtures.schoolhouse.maximumSeats&&resolvedSchool.seatPolicy.seatUnlockRules[1].educationRequired===policyFixtures.schoolhouse.secondSeatEducation&&resolvedSchool.seatPolicy.seatUnlockRules[1].graduationsRequired===policyFixtures.schoolhouse.secondSeatGraduations&&same(resolvedSchool.developmentDomains.map(item=>item.graduationRequirement),policyFixtures.schoolhouse.domainRequirements));
record('candidate-schoolhouse-mentor-graduation',same(candidatePolicy.phase19SchoolhousePolicy.mentorPolicy.intimacyBands.map(item=>[item.minimumInclusive,item.developmentBonus,item.rewardGoldBonus]),policyFixtures.schoolhouse.mentorBands)&&candidatePolicy.phase19SchoolhousePolicy.graduationPolicy.offerVersion===policyFixtures.schoolhouse.graduation.offerVersion&&candidatePolicy.phase19SchoolhousePolicy.graduationPolicy.giftReward===policyFixtures.schoolhouse.graduation.gift&&candidatePolicy.phase19SchoolhousePolicy.graduationPolicy.relicStoneReward===policyFixtures.schoolhouse.graduation.relicStones&&candidatePolicy.phase19SchoolhousePolicy.graduationPolicy.educationDelta===policyFixtures.schoolhouse.graduation.education);
record('candidate-fixed-table-52-continuous',candidateTable.tableId==='reward-table.phase-18-19.structural-rate.candidate-v1'&&candidateTable.productionEnabled===false&&candidateTable.bands.length===policyFixtures.simulation.rewardBandCount&&candidateTable.bands[0].minimumGoldPerHourInclusive===0&&candidateTable.bands.at(-1).maximumGoldPerHourInclusive===null&&candidateTable.bands.slice(1).every((item,index)=>candidateTable.bands[index].maximumGoldPerHourInclusive+1===item.minimumGoldPerHourInclusive));
record('candidate-simulation-five-by-five',same(candidateSimulation.profiles.map(item=>item.profileId),policyFixtures.simulation.profileIds)&&candidateSimulation.profiles.every(item=>same(item.horizons.map(value=>value.days),policyFixtures.simulation.horizonDays))&&candidateSimulation.boundaryAdjacentCases.length===policyFixtures.simulation.boundaryCaseCount);
record('candidate-simulation-guardrails',candidateSimulation.guardrailEvaluation.maximumProfileRecurringShareBasisPoints<=policyFixtures.guardrails.maximumRecurringBasisPoints&&candidateSimulation.guardrailEvaluation.maximumThirtyDayTotalActiveShareBasisPoints<=policyFixtures.guardrails.maximumThirtyDayTotalActiveBasisPoints&&candidateSimulation.guardrailEvaluation.passiveProductionMutations===policyFixtures.guardrails.passiveMutationCount&&candidateSimulation.guardrailEvaluation.familyAssignmentMutations===policyFixtures.guardrails.familyAssignmentMutationCount&&candidateSimulation.headroom.estimatedTotalFacilitySaveBytes<policyFixtures.guardrails.maximumSaveBytes&&candidateSimulation.headroom.safeIntegerHeadroomFactor>=policyFixtures.guardrails.minimumSafeIntegerHeadroomFactor);
record('candidate-tutorial-cast-fixture-coverage',candidateTutorials.entries.length===policyFixtures.coverage.tutorialIds&&candidateCast.entries.length===policyFixtures.coverage.actorIds&&candidateFixtures.cases.length===policyFixtures.coverage.policyFixtures&&candidateTutorials.deliveryRules.maximumNewTutorialSegmentsPerFacilityVisit===1&&candidateTutorials.deliveryRules.featureBlocking===false&&candidateCast.runtimeRules.lockedFellowsExcluded===true&&candidateCast.runtimeRules.playerWayfarerIsSeparateAndNeverFacilityScheduled===true);

const validatorOptions={cwd:ROOT,encoding:'utf8',timeout:120000};
const designRun=spawnSync('/usr/bin/python3',['design/phase-18-19/validate.py'],validatorOptions);
...
const policyRun=spawnSync('/usr/bin/python3',['design/phase-18-19-policy-candidate/validate.py'],validatorOptions);
const candidatePolicyFailIds=policyRun.stdout.split('\n').filter(line=>line.startsWith('FAIL ')).map(line=>line.split(' — ')[0].slice(5));
record('candidate-policy-validator-detects-only-approved-successor',policyRun.status===1&&same(candidatePolicyFailIds,['achievement-source-ids-no-released-collision'])&&policyRun.stdout.includes('SUMMARY 65/66 checks; 86/86 null slots; 52 bands; 5 profiles x 5 horizons; 153 boundary cases; 9 tutorials; 10 actors; 52 fixtures'),{failures:candidatePolicyFailIds,summary:policyRun.stdout.trim().split('\n').at(-1)});
const approvedRun=spawnSync('/usr/bin/python3',['design/phase-18-19-policy-approved/validate.py'],validatorOptions);
record('approved-policy-validator-16-of-16',approvedRun.status===0&&approvedRun.stdout.includes('SUMMARY 16/16 approved-policy checks; 52 bands; 9 tutorials; 10 actors; public release false'),approvedRun.stdout.trim().split('\n').at(-1));
...
record('runner-actual-dom-contract',['document.querySelector','document.querySelectorAll','getBoundingClientRect','getComputedStyle','document.activeElement','KeyboardEvent','scrollWidth','source.matchAll','candidateReducedCssContract','rootMarkerAttribute'].every(value=>realmSource.includes(value)));
record('runner-core-behavior-coverage',['apothecary-recheck-nonterminal-rewardless','apothecary-claim-exactly-once','schoolhouse-lessons-bank-without-pupil','schoolhouse-engagement-resumes-exactly','family-bytes-never-mutated-by-mentor','graduation-v2-exactly-once-seat-release','two-client-${kind}-one-winner','v2-archive-fold-preserves-domain-authority','offline-cap-banks-only-no-agency','player-wayfarer-separate-from-collectible-rosters','runtime-marker-and-static-css-reduced-motion-contract','passive-baseline-same-time-normalized-contract','passive-original-four-same-time-semantics-preserved','actual-dom-apothecary-focus-keyboard-escape-controls','apothecary-pointer-discover-introduce-atomic-focus-log-replay','schoolhouse-keyboard-discover-introduce-atomic-focus-log-replay','facility-introduction-three-stage-rollback-atomic','after-unlock','after-projection','after-outcome','probeFacilityIntroductionFailure','data-phase1819-tutorial-log-row','liveNamedPatientNoop'].every(value=>realmSource.includes(value)));
...
if(PACKAGE_ONLY){
  ...
  candidate.push(['candidate-predecessor-v2-runtime-seams',hasBridge&&combined.includes(fixtures.predecessor.phase12ActivationId)&&combined.includes(fixtures.predecessor.phase15ArchiveConfigId)&&combined.includes(fixtures.predecessor.phase17StoryRegistryId)&&combined.includes('passiveBaseline')&&combined.includes('capturedAt')]);
  ...
  candidate.push(['candidate-null-policy-fail-closed-runtime',hasBridge&&combined.includes('policyReport')&&combined.includes('productionEnabled')&&combined.includes('neverProductionFallback')&&combined.includes('privateEnabled')&&combined.includes('private-release-unavailable')]);
  ...
  candidate.push(['candidate-phase18-19-dom-contract',hasBridge&&Object.values(fixtures.requiredDomSelectors).flat().every(selector=>combined.includes(selector.match(/data-[\w-]+/)?.[0]||selector))&&combined.includes(fixtures.reducedMotionContract.rootMarkerAttribute)&&combined.includes('prefers-reduced-motion')]);
  record('candidate-boundary-registry-exact',same(candidate.map(([id])=>id),fixtures.requiredCandidateBoundaryIds),candidate.map(([id])=>id));
...
const owned=item=>['docs/PHASE_18_19_INDEPENDENT_QA_CONTRACT.md','docs/PHASE_18_19_INDEPENDENT_QA_RESULT.md'].includes(item)||item.startsWith('qa/phase-18-19-independent/')||item.startsWith('qa/phase-18-19-runtime/')||item.startsWith('design/phase-18-19-policy-candidate/');
const commits=git(['log','--format=%H',`${BASE}..HEAD`,'--','docs/PHASE_18_19_INDEPENDENT_QA_CONTRACT.md','docs/PHASE_18_19_INDEPENDENT_QA_RESULT.md','qa/phase-18-19-independent','design/phase-18-19-policy-candidate']).trim().split('\n').filter(Boolean),violations=[];
