...
check('mixed-board-fixture-is-presentation-safe',()=>{assert.match(html,/phaseFifteenConfigureFixtureInState[\s\S]*?story\.book1\.prologue\.council[\s\S]*?pendingSceneIds=\[\][\s\S]*?tutorial\.facility\.board\.discover-hotspots'\]='completed'/);assert.match(html,/PHASE_THIRTEEN_UI\.sceneId=null/);assert.match(html,/function phaseFifteenMaybeAutoTutorial[\s\S]*?phaseFifteenTutorial\(id,'open',\{present:true\}\)/);assert.match(html,/function phaseFifteenEligibleTutorialIds[\s\S]*?\['unseen','open'\]/)});
