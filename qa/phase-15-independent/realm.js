(() => {
  ...
  function runSuite(){
    ...
    (async()=>{
      ...
      let focusSheetOk=false,focusReturnOk=false,escapeNeutral=false,sheetEvidence={openerId:opener?.dataset.phase15FacilityId||null,openerState:opener?.dataset.phase15State||null,onclickPresent:typeof opener?.onclick==='function',overlayBefore:document.querySelector('[data-overlay]')?.innerHTML?.slice(0,500)||null,overlayKind:null,sheetPresent:false,activeBeforeEscape:null,error:null};
      ...
      add('actual-dom-sheet-focus-and-escape',focusSheetOk&&focusReturnOk&&escapeNeutral,{focusSheetOk,focusReturnOk,escapeNeutral,activeAfterEscape:document.activeElement?.outerHTML?.slice(0,240),evidence:sheetEvidence});
