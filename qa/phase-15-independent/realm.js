(() => {
  ...
  function runSuite(){
    ...
    (async()=>{
      ...
      add('archive-checkpoint-lineage-canonical',archiveAfter?.archiveCheckpoint?.receiptCount===128&&typeof archiveAfter?.archiveCheckpoint?.identity==='string'&&archiveAfter.archiveCheckpoint.identity.length>0&&archiveAfter?.predecessorClaimedOfferIds?.length>=128&&archiveAfter?.recentReceipts?.every((item,index)=>item.sequence===archiveAfter.throughSequence+index+1)&&archiveAfter.throughSequence+archiveAfter.recentReceipts.length===archiveAfter.nextSequence,archiveAfter);
      ...
      add('locked-fellows-never-speak',selectedSpeakers.every(value=>{const [roster,id]=String(value).split(':');return roster!=='fellow'||joined.includes(id)}),{selectedSpeakers,joined});
