(() => {
  'use strict';
  const CHANNEL='everstead-phase-14-independent';
  ...
  function installRuntime(config){
    ...
    Object.assign(window,{
      __P14I_CONFIG__:config,__P14I_SLOTS__:slots,__P14I_WRITES__:writes,__P14I_ERRORS__:errors,
      __P14I_NATIVE_ACCESSES__:nativeStorageAccesses,__P14I_NATIVE_INSTRUMENTED__:nativeStorageInstrumented
    });
    window.__EVERSTEAD_RUNTIME__={
      ...
      random:()=>.375,
      confirm:()=>true,
      ids:{save:()=>`save-p14i-${++saveIndex}`,transaction:()=>`tx-p14i-${++transactionIndex}`},
      qa:{allowDestructive:true,isolatedStorage:true}
    };
  }

  function runSuite(){
    const config=window.__P14I_CONFIG__,q=config.fixtures,p13=config.phase13,f={...p13,bridgeVersion:q.bridgeVersion,frozenNow:q.frozenNow,laterNow:q.frozenNow+q.offlineCapMs+3600000,offlineCapMs:q.offlineCapMs,fixtures:{...p13.fixtures,...q.saveFixtures}},results=[];
    ...
    const send=()=>parent.postMessage({channel:config.channel,nonce:config.nonce,results,errors:window.__P14I_ERRORS__.slice()},'*');
    ...
    (async()=>{
      ...
      add('bounded-legacy-slice-one-track-one-feat-one-claim',(definitions.legacy?.tracks||[]).length===q.legacyScope.phase13ContinuingTracks&&(definitions.legacy?.feats||[]).length===q.legacyScope.phase13OneTimeFeats&&(definitions.legacy?.manualClaims||[]).length===q.legacyScope.phase13ManualClaims&&q.legacyScope.phase15ProductionEnabled===false&&q.legacyScope.phase15EconomyApproved===false,{legacy:definitions.legacy,scope:q.legacyScope});
      const validationDefinition=definitions.validation?.phase14;
      add('phase14-validation-definition-contract',validationDefinition?.policy===q.rewardImpactPolicy&&same(validationDefinition?.profiles,q.pacingProfiles)&&Object.values(q.saveFixtures).every(id=>(validationDefinition?.fixtureIds||[]).includes(id)),validationDefinition);
      ...
      add('fresh-first-covenant-once',freshOpen?.ok===true&&freshDerived?.story?.activeId===q.phase13StoryIds[0]&&historyCount(q.phase13StoryIds[0],freshDerived)+pendingCount(q.phase13StoryIds[0],freshDerived)+(freshDerived?.story?.activeId===q.phase13StoryIds[0]?1:0)===1,freshDerived?.story);
      ...
      const midgame=call('destructive','resetFixture',q.saveFixtures.midgame),midgameValidation=call('read','validate'),midgameDerived=call('read','derive');
      const midgameHistory=q.phase13StoryIds.slice(0,3).every(id=>historyCount(id,midgameDerived)===1),midgameFuture=q.phase13StoryIds.slice(3).every(id=>historyCount(id,midgameDerived)===0);
      add('midgame-valid-and-measured',midgame?.ok===true&&midgameValidation?.ok===true&&midgameDerived?.phase14Validation?.profileId==='midgame'&&midgameHistory&&midgameFuture,{midgame,validation:midgameValidation,story:midgameDerived?.story,measurement:midgameDerived?.phase14Validation});

      const migrated=call('destructive','resetFixture',q.saveFixtures.migrated),migratedValidation=call('read','validate'),migratedDerived=call('read','derive'),migration=migratedDerived?.migration;
      add('migrated-baseline-honest',migrated?.ok===true&&migratedValidation?.ok===true&&migration?.historicalStoryBaseline==='unknown'&&migration?.historicalTutorialBaseline==='unknown'&&migration?.inventedCompletionCount===0&&migration?.inventedClaimCount===0&&migration?.recapAutoPresentedThisSession<=1,{migrated,validation:migratedValidation,migration});

      call('destructive','resetFixture',q.saveFixtures.fresh);const corruptBefore=call('read','raw'),corruptRevision=revision(call('read','snapshot')),corrupt=call('destructive','resetFixture',q.saveFixtures.corrupt);
      add('corrupt-fixture-refused',corrupt?.ok===false&&Number(corrupt.writes||0)===0&&same([...(corrupt.failedChecks||[])].sort(),q.corruptChecks)&&call('read','raw')===corruptBefore&&revision(call('read','snapshot'))===corruptRevision,corrupt);
      ...
      add('claim-carryover-and-presentation',Number.isSafeInteger(firstClaim?.carriedProgress)&&firstClaim.carriedProgress>=0&&firstClaim?.presentation?.manual===true&&firstClaim?.presentation?.individual===true&&firstClaim?.presentation?.confirmationRequired===false&&firstClaim?.presentation?.rewardSummaryEquivalent===true,firstClaim);
      const claimRender=call('read','renderModel');
      add('claim-presentation-device-safe',claimRender?.claim?.open===true&&claimRender?.claim?.rewardSummaryExact===true&&claimRender?.claim?.horizontalOverflow===false&&claimRender?.claim?.controlsClipped===false&&claimRender?.claim?.reducedMotion===config.viewport.reducedMotion,claimRender?.claim);
      ...
      add('manual-claim-reload-write-free',reloadDuplicate?.ok===false&&call('read','raw')===reloadRaw&&same(call('read','derive')?.resources,claimedResources)&&call('read','derive')?.legacy?.carriedProgress===firstClaim?.carriedProgress&&Number(reloadDuplicate.writes||0)===0,reloadDuplicate);
      ...
      add('claim-two-client-one-winner',race?.ok===true&&race.winnerCount===1&&race.loserCount===1&&race.receiptCount===1&&race.rewardApplications===1&&race.losingWrites===0&&race.storyTriggerDuplicateCount===0&&race.rankTriggerDuplicateCount===0&&race.finalValid===true,race);
      ...
      call('destructive','reload');const tutorialAfterReload=tutorialById(tutorialId,call('read','derive'));
      add('tutorial-completion-persists-reward-free',tutorialAfterReload?.status==='completed'&&tutorialAfterReload?.completionReceiptCount===1&&same(call('read','derive')?.resources,beforeTutorialComplete?.resources)&&same(call('read','derive')?.claims,beforeTutorialComplete?.claims),tutorialAfterReload);
      ...
      call('destructive','resetFixture',q.saveFixtures.offline);const offlineBefore=call('read','derive'),offlineStory=JSON.stringify(offlineBefore?.story),offlineTutorial=JSON.stringify(offlineBefore?.tutorials),offlineClaims=JSON.stringify(offlineBefore?.claims),offlineAdvance=call('destructive','advanceOffline',q.offlineCapMs*2),offlineAfter=call('read','derive');
      add('offline-cap-preserved',offlineAdvance?.ok===true&&offlineAdvance?.appliedMs===q.offlineCapMs&&JSON.stringify(offlineAfter?.story)===offlineStory&&JSON.stringify(offlineAfter?.tutorials)===offlineTutorial&&JSON.stringify(offlineAfter?.claims)===offlineClaims&&Number(offlineAfter?.resources?.gold)>=Number(offlineBefore?.resources?.gold),{advance:offlineAdvance,before:{story:offlineBefore?.story,tutorials:offlineBefore?.tutorials,claims:offlineBefore?.claims},after:{story:offlineAfter?.story,tutorials:offlineAfter?.tutorials,claims:offlineAfter?.claims}});

      const pacingReports=[];
      for(const profileId of q.pacingProfiles){
        const fixtureId=profileId==='fresh'?q.saveFixtures.fresh:profileId==='midgame'?q.saveFixtures.midgame:q.saveFixtures.established;
        const reset=call('destructive','resetFixture',fixtureId),report=call('read','derive')?.phase14Validation;
        if(reset?.ok===true&&report)pacingReports.push(report);
      }
      ...
      const pacingSafe=pacingReports.every(report=>integerFields.every(field=>Number.isSafeInteger(report[field])&&report[field]>=0)&&Array.isArray(report.introducedStoryIds)&&Array.isArray(report.introducedTutorialIds)&&Array.isArray(report.pendingManualClaimIds)&&report.deadlockFree===true);
      const freshPacing=pacingReports.find(item=>item.profileId==='fresh');
      add('pacing-three-profiles',same(pacingReports.map(item=>item.profileId),q.pacingProfiles)&&pacingSafe&&freshPacing?.stage1Reachable===true&&freshPacing?.affordableConsecutiveFirstClears>=1&&freshPacing?.affordableConsecutiveFirstClears<freshPacing?.totalStageCount&&freshPacing?.stopReason==='gold',pacingReports);
      const impactRows=pacingReports.flatMap(report=>report.rewardImpact||[]),impactSafe=impactRows.length>0&&impactRows.every(item=>typeof item.rewardId==='string'&&Number.isSafeInteger(item.rewardPolicyVersion)&&item.rewardPolicyVersion>=1&&item.canonical===true&&same(item.profileIds,q.pacingProfiles)&&item.postClaimDelta&&Object.values(item.postClaimDelta).every(value=>Number.isSafeInteger(value))&&Array.isArray(item.forbiddenSystems)&&item.forbiddenSystems.length===0);
      add('reward-impact-measurement-only',validationDefinition?.policy===q.rewardImpactPolicy&&pacingReports.every(item=>item.policy===q.rewardImpactPolicy&&item.simulationWrites===0)&&impactSafe,{definition:validationDefinition,reports:pacingReports});
      ...
      const accessibility=render?.accessibility,supportedControls=accessibility?.supportedControls||[];
      add('keyboard-and-focus-contract',accessibility?.keyboardOrderValid===true&&accessibility?.focusReturnValid===true&&accessibility?.escapeNeutral===true&&['next','back','skip','log','close','replay','claim'].every(id=>supportedControls.includes(id)),accessibility);
      ...
      add('native-storage-instrumented',window.__P14I_NATIVE_INSTRUMENTED__===true);
      add('zero-native-storage-accesses',window.__P14I_NATIVE_ACCESSES__.length===0,window.__P14I_NATIVE_ACCESSES__);
      add('zero-warning-error-console',window.__P14I_ERRORS__.length===0,window.__P14I_ERRORS__);
      send();
