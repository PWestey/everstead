...
const floatBits=value=>{const buffer=new ArrayBuffer(8),view=new DataView(buffer);view.setFloat64(0,value,false);return view.getBigUint64(0,false)},orderedFloatBits=value=>{const bits=floatBits(value),sign=1n<<63n,mask=(1n<<64n)-1n;return(bits&sign)!==0n?(~bits)&mask:bits|sign};
...
const predecessor=await runEngineProbe(),failures=predecessor.rows.filter(row=>!row.pass),failureIds=failures.map(row=>row.id),faultFailure=failures.find(row=>row.id==='fault-reload-single-gold-settlement');
let faultWithinOneUlp=false,faultDetail=null;
if(faultFailure){try{faultDetail=JSON.parse(faultFailure.detail);const left=orderedFloatBits(faultDetail.pending),right=orderedFloatBits(faultDetail.expectedFaultPending);faultWithinOneUlp=faultDetail.blocked===null&&(left>right?left-right:right-left)<=1n}catch{faultWithinOneUlp=false}}
const predecessorCompatible=failures.length===1&&failureIds[0]==='idle-cursors-independent-from-gold'||failures.length===2&&failureIds.includes('idle-cursors-independent-from-gold')&&failureIds.includes('fault-reload-single-gold-settlement')&&faultWithinOneUlp;
add('phase10c2-single-explicit-supersession',predecessorCompatible,{failureIds,faultDetail,faultWithinOneUlp});
