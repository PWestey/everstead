...
function runRealm(options={}){
  ...
  const node=()=>{const value={innerHTML:'',dataset:{},style:{},className:''};value.classList={add(name){if(!value.className.split(/\s+/).includes(name))value.className=(value.className+' '+name).trim()},remove(name){value.className=value.className.split(/\s+/).filter(item=>item&&item!==name).join(' ')},contains(name){return value.className.split(/\s+/).includes(name)}};return value},nodes={'#app':node(),'#overlay':node(),'#toast':node(),'.campaign-walk':node()};
...
check('reduced-motion-runtime-contract',source.includes("MATCH_MEDIA_INPUT?.('(prefers-reduced-motion: reduce)')?.matches===true")&&source.includes('if(runtimePrefersReducedMotion())campaignResultModal(receipt)'));
const reducedPresentation=runRealm({...freshOptions,reducedMotion:true,deferTimers:true}),reducedResult=internal(reducedPresentation,"run('broken-roads-1',{confirmed:true})");
check('reduced-motion-run-commits',reducedResult.ok===true&&active(reducedPresentation)?.fellowCampaign.lastReceipt?.stageId==='broken-roads-1');
check('reduced-motion-result-synchronous',reducedPresentation.nodes['#overlay'].innerHTML.includes('Stage cleared')&&reducedPresentation.nodes['#overlay'].innerHTML.includes('Village Toll'));
check('reduced-motion-no-running-class',!reducedPresentation.nodes['.campaign-walk'].classList.contains('running'));
check('reduced-motion-no-presentation-timer',reducedPresentation.timers.size===0,JSON.stringify([...reducedPresentation.timers.keys()]));
const normalPresentation=runRealm({...freshOptions,reducedMotion:false,deferTimers:true}),normalResult=internal(normalPresentation,"run('broken-roads-1',{confirmed:true})"),normalTimer=[...normalPresentation.timers.entries()][0];
check('normal-motion-run-commits',normalResult.ok===true&&active(normalPresentation)?.fellowCampaign.lastReceipt?.stageId==='broken-roads-1');
check('normal-motion-running-class',normalPresentation.nodes['.campaign-walk'].classList.contains('running'));
check('normal-motion-result-delayed',normalPresentation.nodes['#overlay'].innerHTML===''&&normalPresentation.timers.size===1);
if(normalTimer)normalTimer[1]();
check('normal-motion-result-after-delay',normalPresentation.nodes['#overlay'].innerHTML.includes('Stage cleared')&&normalPresentation.nodes['#overlay'].innerHTML.includes('Village Toll'));
...
const protectedIdentityMap=run=>({version:1,backupRawIdentity:rawIdentity(run.slots.get(keys.backupV0)??null),preV2RawIdentity:rawIdentity(run.slots.get(keys.backupV1)??null),preV3RawIdentity:rawIdentity(run.slots.get(keys.backupV2)??null),preV4RawIdentity:rawIdentity(run.slots.get(keys.backupV3)??null),preV5RawIdentity:rawIdentity(run.slots.get(keys.backupV4)??null),preV6RawIdentity:rawIdentity(run.slots.get(keys.backupV5)??null)});
const receiptMatchesProtectedSlots=run=>{const receipt=active(run)?.saveMeta.appliedMigrations.find(item=>item.id==='schema-5-to-6');return JSON.stringify(receipt?.checkpointLineage)===JSON.stringify(protectedIdentityMap(run))};
for(const [label,raw]of predecessorRows){const migrated=runRealm({...freshOptions,activeRaw:raw}),reloaded=runRealm({initialSlots:Object.fromEntries(migrated.slots)});check(`${label}-receipt-matches-live-slots`,receiptMatchesProtectedSlots(migrated));check(`${label}-immediate-reload`,active(reloaded)?.schemaVersion===6&&internal(reloaded,'runtime().blocked')===null&&receiptMatchesProtectedSlots(reloaded),JSON.stringify(internal(reloaded,'runtime().blocked')));const mutation=act(reloaded,'navigate',{view:'more'}),mutatedReload=runRealm({initialSlots:Object.fromEntries(reloaded.slots)});check(`${label}-post-migration-mutation`,mutation.ok===true&&mutation.changed===true);check(`${label}-post-mutation-reload`,active(mutatedReload)?.schemaVersion===6&&internal(mutatedReload,'runtime().blocked')===null&&receiptMatchesProtectedSlots(mutatedReload),JSON.stringify(internal(mutatedReload,'runtime().blocked')))}
...
check('v5-receipt-matches-live-slots',receiptMatchesProtectedSlots(directFive));
const directFiveReloadForIdentity=runRealm({initialSlots:Object.fromEntries(directFive.slots)}),directFiveMutation=act(directFiveReloadForIdentity,'navigate',{view:'more'}),directFiveMutatedReload=runRealm({initialSlots:Object.fromEntries(directFiveReloadForIdentity.slots)});
check('v5-immediate-reload',active(directFiveReloadForIdentity)?.schemaVersion===6&&internal(directFiveReloadForIdentity,'runtime().blocked')===null&&receiptMatchesProtectedSlots(directFiveReloadForIdentity));
check('v5-post-migration-mutation',directFiveMutation.ok===true&&directFiveMutation.changed===true);
check('v5-post-mutation-reload',active(directFiveMutatedReload)?.schemaVersion===6&&internal(directFiveMutatedReload,'runtime().blocked')===null&&receiptMatchesProtectedSlots(directFiveMutatedReload));
const migrationFaults=[['staging-write','setItem'],['staging-verify','getItem'],['active-conflict-check','getItem'],['active-write','setItem'],['active-verify','getItem'],['staging-cleanup-owner','getItem'],['staging-cleanup','removeItem']];
for(const [label,raw]of [...predecessorRows.map(([name,value])=>[name,value]),['v5',schemaFiveRaw]])for(const [step,operation]of migrationFaults){const interrupted=runRealm(label==='v5'?{initialSlots:schemaFiveSlots,fault:{enabled:true,operation,step}}:{...freshOptions,activeRaw:raw,fault:{enabled:true,operation,step}}),retry=runRealm({initialSlots:Object.fromEntries(interrupted.slots)}),finalReload=runRealm({initialSlots:Object.fromEntries(retry.slots)}),state=active(finalReload);check(`${label}-${step}-fault-fired`,interrupted.fault.remaining===0);check(`${label}-${step}-retry-current`,state?.schemaVersion===6&&internal(finalReload,'runtime().blocked')===null,JSON.stringify({retryBlocked:internal(retry,'runtime().blocked'),finalBlocked:internal(finalReload,'runtime().blocked'),thrown:finalReload.thrown?.message}));check(`${label}-${step}-retry-clean`,!finalReload.slots.has(keys.staging)&&state?.saveMeta.appliedMigrations.filter(item=>item.id==='schema-5-to-6').length===1);check(`${label}-${step}-receipt-live-exact`,receiptMatchesProtectedSlots(finalReload))}
...
const semanticMutation=raw=>{const state=JSON.parse(raw);state.gold=(state.gold??0)+777;state.saveMeta.source='foreign-semantic-checkpoint';return JSON.stringify(state)},currentResetBase=Object.fromEntries(migratedOne.slots),resetArchiveVariants=[['backup-whitespace',keys.backupV0,(currentResetBase[keys.backupV0]??'')+' '],['backup-semantic',keys.backupV0,semanticMutation(currentResetBase[keys.backupV0])],['backup-malformed',keys.backupV0,'{"malformed":'],['pre-v6-whitespace',keys.backupV5,(currentResetBase[keys.backupV5]??'')+' '],['pre-v6-semantic',keys.backupV5,semanticMutation(currentResetBase[keys.backupV5])],['pre-v6-malformed',keys.backupV5,'{"malformed":']];
for(const [label,slot,tamperedRaw]of resetArchiveVariants){const slots={...currentResetBase,[slot]:tamperedRaw,[keys.staging]:null},run=runRealm({initialSlots:slots}),protectedBefore=snapshot(run),preReset=stateFrom(slots[keys.active]);check(`current-safe-reset-${label}-starts-blocked`,internal(run,'runtime().blocked')!==null&&writes(run)===0);internal(run,'reset()');const state=active(run),marker=state?.saveMeta.retainedCheckpointLineage;check(`current-safe-reset-${label}-commits`,state?.saveMeta.source==='safe-reset'&&marker?.preResetSaveId===preReset?.saveMeta.saveId&&marker?.preResetRevision===preReset?.saveMeta.revision&&internal(run,'runtime().blocked')===null);check(`current-safe-reset-${label}-preserves-archive`,run.slots.get(slot)===protectedBefore[slot]);const reload=runRealm({initialSlots:Object.fromEntries(run.slots)}),mutation=act(reload,'navigate',{view:'more'}),mutatedReload=runRealm({initialSlots:Object.fromEntries(reload.slots)});check(`current-safe-reset-${label}-immediate-reload`,active(reload)?.schemaVersion===6&&internal(reload,'runtime().blocked')===null&&!reload.slots.has(keys.staging));check(`current-safe-reset-${label}-post-mutation-reload`,mutation.ok===true&&active(mutatedReload)?.schemaVersion===6&&internal(mutatedReload,'runtime().blocked')===null&&mutatedReload.slots.get(slot)===protectedBefore[slot])}
...
for(const [step,operation]of safeResetFaults){const tampered=(currentResetBase[keys.backupV5]??'')+' ',slots={...currentResetBase,[keys.backupV5]:tampered,[keys.staging]:null},run=runRealm({initialSlots:slots});run.fault.enabled=true;run.fault.operation=operation;run.fault.step=step;run.fault.remaining=1;internal(run,'reset()');check(`current-safe-reset-${step}-fault-fired`,run.fault.remaining===0);let retry=runRealm({initialSlots:Object.fromEntries(run.slots)});if(internal(retry,'runtime().blocked')!==null&&(retry.slots.get(keys.staging)??null)===null)internal(retry,'reset()');const finalReload=runRealm({initialSlots:Object.fromEntries(retry.slots)}),state=active(finalReload);check(`current-safe-reset-${step}-retry-current`,state?.schemaVersion===6&&state.saveMeta.retainedCheckpointLineage?.kind==='safe-reset-retained-checkpoints'&&internal(finalReload,'runtime().blocked')===null,JSON.stringify(internal(finalReload,'runtime().blocked')));check(`current-safe-reset-${step}-retry-clean`,(finalReload.slots.get(keys.staging)??null)===null&&finalReload.slots.get(keys.backupV5)===tampered)}
