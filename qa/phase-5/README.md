...
- `verify.mjs` covers the ten stages, Player Rank, exact reward formulas, deterministic receipts, Story migration, schema0–5 post-write slot attestations and every migration interruption boundary, current-base reset archives (including malformed/foreign bytes), all eight post-write boot/read faults, motion-sensitive result presentation, retired-mode quarantine, malformed-save fail-closure, and exact fixture rollback.
