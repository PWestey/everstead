...
function instrument(script) {
  const hook = `globalThis.__P2_INTERNAL__=Object.freeze({
    ...
    runtime:()=>clone({persistedRaw:PERSISTED_RAW,persistedIdentity:PERSISTED_IDENTITY,blocked:PERSISTENCE_BLOCKED,notice:PERSISTENCE_NOTICE,outcome:PERSISTENCE_OUTCOME,stale:PERSISTENCE_STALE,writeInProgress:PERSISTENCE_WRITE_IN_PROGRESS}),
...
const safeIntegerState=clone(activeState(fresh));safeIntegerState.fellows.cael.exp=Number.MAX_SAFE_INTEGER;safeIntegerState.fellows.cael.level=120;safeIntegerState.fellows.cael.shards=Number.MAX_SAFE_INTEGER;
const safeIntegerRaw=JSON.stringify(safeIntegerState),safeIntegerRun=runRealm({activeRaw:safeIntegerRaw,backupRaw:safeIntegerRaw,preV2BackupRaw:null,preV3BackupRaw:null,stagingRaw:null});
check('current-safe-integer-boundary-accepted',activeState(safeIntegerRun)?.fellows.cael.exp===Number.MAX_SAFE_INTEGER&&activeState(safeIntegerRun)?.fellows.cael.shards===Number.MAX_SAFE_INTEGER);
for(const [label,field] of [['exp','exp'],['shards','shards']]){const value=clone(activeState(fresh));value.fellows.cael[field]=Number.MAX_SAFE_INTEGER+1;if(field==='exp')value.fellows.cael.level=120;const raw=JSON.stringify(value),run=runRealm({activeRaw:raw,backupRaw:null,preV2BackupRaw:null,preV3BackupRaw:null,stagingRaw:null});check(`current-unsafe-${label}-refused-zero-write`,activeRaw(run)===raw&&writeCount(run)===0&&Object.values(slotSnapshot(run)).slice(1).every(item=>item===null)&&/safe integer precision/i.test(run.nodes['#app'].innerHTML));check(`validation-unsafe-${label}-refused`,evaluate(run,`__P2_INTERNAL__.validation(${JSON.stringify(value)}).ok`)===false)}
check('migration-huge-training-normalizes-safe',Number.isSafeInteger(activeState(runRealm({activeRaw:JSON.stringify({...JSON.parse(legacyRaw),fellows:{...JSON.parse(legacyRaw).fellows,cael:{...JSON.parse(legacyRaw).fellows.cael,training:Number.MAX_SAFE_INTEGER+1}}}),backupRaw:null,preV2BackupRaw:null,preV3BackupRaw:null,stagingRaw:null}))?.fellows.cael.exp));
...
for(const [label,totalPower] of [['cap-minus-epsilon',71999.999],['cap-boundary',72000],['cap-plus-epsilon',72000.001]]){const value=evaluate(powerRun,`__P2_INTERNAL__.efficiencyTotal(${totalPower},12000,30000)`),ratio=Math.max(0,totalPower/30000-1),discount=Math.min(.35,ratio*.25);check(`efficiency-${label}-actual-selector`,value.valid&&value.discountRate===discount&&value.effectiveCost===Math.ceil(12000*(1-discount)),JSON.stringify(value))}
...
const unrelatedSchemaTwoValue=clone(schemaTwoValue);unrelatedSchemaTwoValue.gold++;
for(const [activeLabel,active] of [['schema1',schemaOneRaw],['schema2',schemaTwoRaw]])for(const [checkpointLabel,checkpoint] of [['malformed','{malformed-v2-checkpoint'],['valid-unrelated',JSON.stringify(unrelatedSchemaTwoValue)]]){const run=runRealm({activeRaw:active,backupRaw:null,preV2BackupRaw:null,preV3BackupRaw:checkpoint,stagingRaw:null}),expected={[keys.active]:active,[keys.backupV0]:null,[keys.backupV1]:null,[keys.backupV2]:checkpoint,[keys.staging]:null};check(`preflight-${activeLabel}-${checkpointLabel}-v2-zero-write`,writeCount(run)===0,JSON.stringify(run.storageLog.filter(([operation])=>operation==='set'||operation==='remove')));check(`preflight-${activeLabel}-${checkpointLabel}-five-slots-exact`,JSON.stringify(slotSnapshot(run))===JSON.stringify(expected));}
const foreignSourceSchemaTwo=clone(schemaTwoValue);foreignSourceSchemaTwo.saveMeta.source='foreign-source';const foreignSourceSchemaTwoRaw=JSON.stringify(foreignSourceSchemaTwo),foreignSourceRun=runRealm({activeRaw:schemaOneRaw,backupRaw:null,preV2BackupRaw:null,preV3BackupRaw:foreignSourceSchemaTwoRaw,stagingRaw:null});
check('preflight-foreign-source-v2-refused-zero-write',writeCount(foreignSourceRun)===0&&activeRaw(foreignSourceRun)===schemaOneRaw&&foreignSourceRun.slots.get(keys.backupV2)===foreignSourceSchemaTwoRaw&&!activeState(foreignSourceRun));
check('preflight-foreign-source-all-lower-slots-empty',!foreignSourceRun.slots.has(keys.backupV0)&&!foreignSourceRun.slots.has(keys.backupV1)&&!foreignSourceRun.slots.has(keys.staging));
const rollbackMigrationNow=Date.parse(scenarios.frozenNow),rollbackFuture=rollbackMigrationNow+86_400_000,rollbackSchemaTwo=clone(schemaTwoValue),rollbackDay=new Date(rollbackFuture-scenarios.timezoneOffsetMinutes*60_000).toISOString().slice(0,10);rollbackSchemaTwo.saveMeta.updatedAt=rollbackFuture;rollbackSchemaTwo.lastSeen=rollbackFuture;rollbackSchemaTwo.lastGoldAt=rollbackFuture;rollbackSchemaTwo.day=rollbackDay;rollbackSchemaTwo.patrolBank=0;rollbackSchemaTwo.pendingGold=.625;for(const building of Object.values(rollbackSchemaTwo.buildings)){building.boost=.30;building.boostDay=rollbackDay}const rollbackHabit=rollbackSchemaTwo.oaths.find(oath=>oath.type==='habit');rollbackHabit.count=5;const rollbackSchemaTwoRaw=JSON.stringify(rollbackSchemaTwo),rollbackRewards={gold:rollbackSchemaTwo.gold,prosperity:rollbackSchemaTwo.prosperity,bonds:Object.fromEntries(Object.entries(rollbackSchemaTwo.fellows).map(([id,value])=>[id,value.bond]))};
const rollbackInterrupted=runRealm({activeRaw:rollbackSchemaTwoRaw,backupRaw:null,preV2BackupRaw:null,preV3BackupRaw:null,stagingRaw:null,now:rollbackMigrationNow,fault:{enabled:true,operation:'setItem',step:'active-write'}}),rollbackRetry=runRealm({initialSlots:Object.fromEntries(rollbackInterrupted.slots),now:rollbackMigrationNow+60_000}),rollbackState=activeState(rollbackRetry),rollbackReceipt=rollbackState?.saveMeta.appliedMigrations.find(item=>item.id==='schema-2-to-3');
check('rollback-migration-retry-current',rollbackState?.schemaVersion===3&&!rollbackRetry.slots.has(keys.staging)&&rollbackRetry.slots.get(keys.backupV2)===rollbackSchemaTwoRaw);
check('rollback-migration-metadata-monotonic',rollbackState?.saveMeta.updatedAt===rollbackFuture&&rollbackReceipt?.appliedAt===rollbackMigrationNow&&rollbackState?.saveMeta.appliedMigrations.filter(item=>item.id==='schema-2-to-3').length===1);
check('rollback-migration-time-day-state-preserved',rollbackState?.lastSeen===rollbackFuture&&rollbackState?.lastGoldAt===rollbackFuture&&rollbackState?.day===rollbackDay&&rollbackState?.patrolBank===0&&rollbackState?.pendingGold===.625&&rollbackState?.oaths.find(oath=>oath.id===rollbackHabit.id)?.count===5&&Object.values(rollbackState?.buildings??{}).every(building=>building.boost===.30&&building.boostDay===rollbackDay));
check('rollback-migration-no-rewards',rollbackState?.gold===rollbackRewards.gold&&rollbackState?.prosperity===rollbackRewards.prosperity&&JSON.stringify(Object.fromEntries(Object.entries(rollbackState?.fellows??{}).map(([id,value])=>[id,value.bond])))===JSON.stringify(rollbackRewards.bonds));
...
const fixtureTargetState=clone(activeState(fresh));fixtureTargetState.gold+=11;fixtureTargetState.saveMeta.revision++;const fixtureTarget={activeRaw:JSON.stringify(fixtureTargetState),backupRaw:'target-v0',preV2BackupRaw:null,preV3BackupRaw:schemaTwoRaw,stagingRaw:null};
function fixtureFaultRealm(){const run=runRealm({activeRaw:activeRaw(fresh),backupRaw:activeRaw(fresh),preV2BackupRaw:schemaOneRaw,preV3BackupRaw:schemaTwoRaw,stagingRaw:null,qa:{allowDestructive:true,isolatedStorage:true}});run.slots.set(keys.staging,'preimage-stage');return run}
for(const [label,operation,key] of [['active-set','setItem',keys.active],['v0-set','setItem',keys.backupV0],['v1-remove','removeItem',keys.backupV1],['v2-set','setItem',keys.backupV2],['staging-remove','removeItem',keys.staging]]){const run=fixtureFaultRealm(),beforeSlots=slotSnapshot(run),beforeState=evaluate(run,'__EVERSTEAD_QA__.snapshot().state'),beforeRuntime=evaluate(run,'__P2_INTERNAL__.runtime()'),beforeRevision=beforeState.saveMeta.revision,beforeUi=JSON.stringify(Object.fromEntries(Object.entries(run.nodes).map(([selector,node])=>[selector,node.innerHTML]))),logStart=run.persistenceLog.length;Object.assign(run.fault,{enabled:true,remaining:1,operation,key,step:null});const result=evaluate(run,`__EVERSTEAD_QA__.controls.installFixture(${JSON.stringify(fixtureTarget)})`),afterState=evaluate(run,'__EVERSTEAD_QA__.snapshot().state'),afterRuntime=evaluate(run,'__P2_INTERNAL__.runtime()'),afterUi=JSON.stringify(Object.fromEntries(Object.entries(run.nodes).map(([selector,node])=>[selector,node.innerHTML])));check(`fixture-fault-${label}-reported`,result.ok===false);check(`fixture-fault-${label}-five-slots-exact`,JSON.stringify(slotSnapshot(run))===JSON.stringify(beforeSlots));check(`fixture-fault-${label}-runtime-state-revision-ui-exact`,JSON.stringify(afterState)===JSON.stringify(beforeState)&&JSON.stringify(afterRuntime)===JSON.stringify(beforeRuntime)&&afterState.saveMeta.revision===beforeRevision&&afterUi===beforeUi);check(`fixture-fault-${label}-no-normal-boot-save`,run.persistenceLog.slice(logStart).every(entry=>!['staging-write','active-write'].includes(entry.step)));}
const rollbackFailureRun=fixtureFaultRealm(),rollbackFailureSlots=slotSnapshot(rollbackFailureRun),rollbackFailureState=evaluate(rollbackFailureRun,'__EVERSTEAD_QA__.snapshot().state'),rollbackFailureRuntime=evaluate(rollbackFailureRun,'__P2_INTERNAL__.runtime()'),rollbackFailureUi=JSON.stringify(Object.fromEntries(Object.entries(rollbackFailureRun.nodes).map(([selector,node])=>[selector,node.innerHTML]))),rollbackFailureLogStart=rollbackFailureRun.persistenceLog.length;Object.assign(rollbackFailureRun.fault,{enabled:true,remaining:2,operation:'setItem',key:keys.active,step:null});const rollbackFailureResult=evaluate(rollbackFailureRun,`__EVERSTEAD_QA__.controls.installFixture(${JSON.stringify(fixtureTarget)})`);
check('fixture-rollback-failure-explicit',rollbackFailureResult.ok===false&&/rollback failed:/i.test(rollbackFailureResult.error));
check('fixture-rollback-failure-runtime-still-exact',JSON.stringify(evaluate(rollbackFailureRun,'__EVERSTEAD_QA__.snapshot().state'))===JSON.stringify(rollbackFailureState)&&JSON.stringify(evaluate(rollbackFailureRun,'__P2_INTERNAL__.runtime()'))===JSON.stringify(rollbackFailureRuntime)&&JSON.stringify(Object.fromEntries(Object.entries(rollbackFailureRun.nodes).map(([selector,node])=>[selector,node.innerHTML])))===rollbackFailureUi&&rollbackFailureRun.persistenceLog.slice(rollbackFailureLogStart).every(entry=>!['staging-write','active-write'].includes(entry.step)));
check('fixture-rollback-failure-preimage-retained-when-writes-refused',JSON.stringify(slotSnapshot(rollbackFailureRun))===JSON.stringify(rollbackFailureSlots));
