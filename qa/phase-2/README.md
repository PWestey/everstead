...
Frozen candidate totals are 353/353 CLI assertions, 244 inherited Phase 1 assertions plus 12 documented Phase 2 replacements, 13 checksum entries, and 312/312 live browser assertions. Each gate is required to pass twice.

The persistence correction matrix executes malformed, unrelated, and foreign-source v2 checkpoint refusals before any lower-slot write; exact five-slot fixture rollback at every set/remove boundary; explicit rollback-failure reporting; safe-integer import boundaries; rollback-clock migration monotonicity; and the actual Campaign efficiency selector immediately below, at, and above its cap.
