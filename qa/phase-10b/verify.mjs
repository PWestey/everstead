...
const auditOracleSource=simulatorSource.slice(simulatorSource.indexOf('function deriveExpectedLaneClaim('),simulatorSource.indexOf('function initialState(input)')),campaignAuditSeparated=auditOracleSource.length>0&&['initialState(','applyLaneRewards(','powerSnapshot(','levelForExp(','rankForExp(','stableUnit(','campaignEfficiency(','familyRecipient(','upgradeCost(','settleLane('].every(call=>!auditOracleSource.includes(call));
const rewardMutationProofs=Object.fromEntries(['campaign-target','campaign-gift','campaign-sequence','campaign-power-cost','campaign-cross-lane-order','campaign-execution-power-helper','campaign-execution-rng-helper','campaign-execution-access-helper','idle-duplicate','idle-missing','idle-recipient','idle-settlement'].map(mutation=>[mutation,rewardProofMutationProbe(scenarios,mutation)])),rewardMutationsRejected=campaignAuditSeparated&&Object.values(rewardMutationProofs).every(proof=>proof.noDuplicateRewards===false)&&!simulatorSource.includes("'tower:claim-'")&&!simulatorSource.includes("'expedition:claim-'")&&!simulatorSource.includes("'family:claim-'");
const invariantById={
 ...
 'invariant-53-no-double-reward':rewardMutationsRejected&&allBundles.every(bundle=>bundle.safety.resourceAccounting.noDuplicateRewards&&bundle.safety.resourceAccounting.noLostResources&&bundle.safety.resourceAccounting.campaignNoReplay&&bundle.safety.resourceAccounting.rewardedNonFirstClearReceipts.length===0&&bundle.safety.resourceAccounting.duplicateRewardIds.length===0&&bundle.safety.resourceAccounting.lostResources.length===0&&bundle.safety.resourceAccounting.idleRewardProof.valid&&bundle.safety.resourceAccounting.idleRewardProof.claimSchedule.exact&&bundle.safety.resourceAccounting.idleRewardProof.duplicateCount===0&&bundle.safety.resourceAccounting.idleRewardProof.missingCount===0&&bundle.safety.resourceAccounting.idleRewardProof.unexpectedCount===0&&bundle.safety.resourceAccounting.idleRewardProof.awardsMatch&&bundle.progression.campaign.noReplayProof.valid&&bundle.progression.campaign.noReplayProof.globalOrder.valid&&bundle.progression.campaign.noReplayProof.campaignAwardsMatch&&Object.values(bundle.progression.campaign.noReplayProof.lanes).every(lane=>lane.uniqueNaturalIds&&lane.rosterPowerMatches&&lane.blockedZero&&lane.endingMatches&&lane.spendMatches&&lane.postCapNoReceipt)),
