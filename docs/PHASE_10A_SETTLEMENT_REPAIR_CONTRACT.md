...
- Exercise nonzero elapsed Tower-only, Expedition-only, and combined-lane mutations at staging ownership/write/verify, active conflict/write/verify, and cleanup ownership/remove/verify fault boundaries. `staging-new-owner` and `staging-write` faults preserve the exact twelve-slot image, active raw/revision, and gameplay state because no stage became durable. `staging-verify`, `active-conflict-check`, and `active-write` faults preserve active raw/revision and gameplay state while retaining the exact authenticated pending stage for one-time recovery. All propagated pre-commit faults expose the exact accepted persistence-blocked runtime/UI instead of claiming runtime/UI byte identity. `active-verify` may leave the committed target and authenticated stage durable for one-time adoption. Cleanup ownership/remove faults preserve committed active state and may retain staging with the exact accepted notice; cleanup verify may occur after staging is already absent. A same-clock and later-clock reload/retry continues from the authenticated staged/committed cursor without duplicate credited elapsed or lost elapsed.
...
- Run the complete frozen Phase 9 focused CLI verifier and Phase 8 semantic-successor verifier twice on the clean candidate tip. The frozen Phase 9 checksum gate must report exactly 13/14, with only `index.html` superseded from SHA-256 `1e9d22150a5a0d2b2b4fbec403a5a50bf81c3b22153e688b659bda9b6bc67529` to the reviewed Phase 10A artifact and every other row passing. The new Phase 10A checksum gate must pass every row twice.
