...
The kernel is private. It is not assigned to `window` or `globalThis`, made enumerable through another object, added to the QA bridge, exposed through diagnostics, or reachable through a production UI action. QA may expose or instrument it only through the three separate reversible in-memory realms defined under **Production-probe realm separation**, after verifying exact unique anchors and proving byte-exact restoration. No QA hook is present in production bytes.
...
Canonical predecessor-trace serialization represents JavaScript `undefined` with the exact one-key ordinary record `{ "$undefined": true }`. The tag is recognized only by the QA trace serializer/decoder at a trace-schema leaf explicitly declared `encodedValue`; production code, the production artifact, save serialization, and ordinary JSON parsing never interpret it. The record must have prototype `Object.prototype`, exact own key order `['$undefined']`, an enumerable data property with literal value `true`, and no symbol/accessor/foreign field, or decoding fails closed. JavaScript `null` encodes as literal JSON `null` and decodes back to exact `null`; it is never treated as an object or tag.

The tag cannot alias a literal user object, and an array cannot alias an ordinary object with numeric keys. Every non-array ordinary object stored in an `encodedValue` position uses the disjoint canonical form `{ "$object": [["<own key>", <encodedValue>], ...] }`, preserving exact own-key order recursively. Every ordinary dense array uses an ordinary JSON array of recursively encoded `encodedValue` elements; sparse arrays, accessor elements, symbol keys, and extra own properties fail encoding. Decoding an array always creates a fresh dense Array with exact `Array.prototype`, length, element presence, and order. Decoding `$object` always creates a fresh non-array ordinary object with exact `Object.prototype` and the declared string keys in order. Thus `{ "0": "a" }` and `["a"]` remain different types, and an actual user object whose only property is `$undefined:true` serializes as `{ "$object": [["$undefined", true]] }`, never as the undefined tag. A bare `$undefined` key outside an exact tag or a tagged record outside a declared `encodedValue` leaf fails trace validation.

Frozen serializer controls round-trip `undefined`, `null`, empty and nested dense arrays containing `undefined`/`null`, a numeric-key ordinary object beside an equal-looking array, and literal `{ "$undefined": true }`. They require canonical re-encoding bytes, exact prototypes, exact array density/key order, no array/object type alias, literal-tag-object preservation, and fresh identity: no decoded object/array aliases its encoded/source object or any separately decoded copy. These are fixed subassertions of the existing static/startup and exact-shape rows and do not change any category or total. Encoding/decoding occurs only in QA fixture/call-record transport; decoded values are freshly materialized before a realm call and never become expected values by observing candidate output.
...
| Duration Gold | `offlineClaimPreview(at,state)` | for a positive duration, scalar selected by literal `segmentIndex` and `buildingId` from `segments[segmentIndex].values[buildingId]`; for the exact zero-duration trace only, canonical positive zero from the neutral `now === last` return described below |
...
The one zero-duration trace is the exact reachable neutral branch of `offlineClaimPreview` with `now === last`. It freezes the complete predecessor return with empty `segments`, its arithmetic projection is canonical positive zero, and its literal direct-method input is exactly `{rate:7200,durationMs:0}`. Because the accepted predecessor branch performs no per-segment arithmetic, its expected wrapper call record is empty and its aggregate `durationGold` call count is zero. The direct-kernel/reference rows still prove the zero-duration arithmetic independently from that literal input. Only the three positive-duration traces use `segments[segmentIndex].values[buildingId]` as their wrapper projection and expect a corresponding `durationGold` call. This reachability exception changes no trace allocation, row meaning, category total, production behavior, or successor topology.
...
### Production-probe realm separation

The production probe uses exactly three kinds of separate, fresh, reversible QA-only realms. A transformed buffer or realm is never reused by another kind, and no instrumentation output becomes expected authority:

1. **Uninstrumented output realm.** Starting from the exact candidate bytes, replace only the exact unique boot tail with a frozen allowlisted facade that suppresses boot and exposes the seven direct private methods plus the six preserved Gold wrapper selectors required by literal traces. Instances used for the 240 semantic-successor vectors may also expose only that vector's exact inherited Phase 10B-1 selector allowlist. No kernel method body or wrapper delegation is instrumented in this realm. Its direct-method and full-wrapper outputs are the candidate observations used for parity.
2. **Call-trace realm.** Starting again from the original exact candidate bytes, apply the same boot-tail facade and additionally instrument the exact seven private method bodies—this contract freezes method-body instrumentation rather than wrapper-expression instrumentation. Each of the seven literal body anchors must occur exactly once. Instrumentation records only method name, decoded argument value, global call ordinal, and aggregate count without replacing the frozen function object or changing its `name`, `length`, descriptors, constructibility, return, throw, or evaluation order. For every trace, the complete wrapper return or thrown error must be bit-, key-order-, descriptor-, and shape-exact to both the uninstrumented output realm and the frozen predecessor before its call record can be accepted.
3. **One-defect mutation realms.** Every mutation subcase starts again from the original exact candidate bytes, applies the ordinary boot-tail facade, and introduces exactly one defect. The defect transform may alter only one of the seven authorized production windows: the private-kernel insertion or one of the six delegated wrapper bodies. It cannot touch a call site, `familyBuildingAssignment`, `buildingRate`, `nextLocalMidnight`, boot/runtime production logic, persistence, UI, or expected/reference input. Row 48's four structural subcases run in four distinct realms; a deferred-guard mutant must be expressed wholly inside the kernel window.

Before any transform, every realm verifies the original candidate SHA-256 and byte length on raw bytes. It then preflights and freezes its complete literal anchor list—boot tail, exposed declaration selectors, and, when applicable, all seven method-body anchors or the single mutation target—and requires every listed anchor exactly once before replacement. Each realm executes in a fresh context with string and Wasm code generation disabled, freshly cloned then recursively deep-frozen predecessor/reference inputs, poisoned storage/UI/timers/network, no exact native storage, and no inherited Phase 6/7/9 QA hook.

Every replacement records exact byte offsets and original/replacement byte sequences. Applying the inverse replacements in reverse offset order must reproduce the original candidate buffer byte-for-byte, SHA-for-SHA, and length-for-length for each individual realm. An anchor miss/duplicate, transform overlap, inverse mismatch, descriptor drift, output-realm/call-trace-realm result mismatch, unexpected side effect, or cross-realm shared mutable state fails closed. These transforms exist only inside the QA process; production `index.html` contains no facade, trace recorder, mutation switch, export, or test hook.
