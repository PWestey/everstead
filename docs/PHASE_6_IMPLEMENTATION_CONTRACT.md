...
- Tower-clear identity binds save ID + floor + pre-clear sequence + the exact captured pre-clear claimed-interval history + every derived receipt field. A receipt records floor, time, sequence, requirement/total Power, identity, exact Companion EXP/shard maps, nominal Mastery entitlement, pre/post Mastery points, the actual cap-truncated Mastery award, and an exact `preClearClaimedIntervalsByFloor` snapshot with floor keys `1…50`.
- The pre-clear snapshot's safe-integer count sum is the historical idle interval ordinal at that clear; every count is less than or equal to the current cumulative count. Canonical replay of that snapshot plus nominal Tower-clear Mastery through floor `F - 1` derives the receipt's pre-Mastery points exactly. `post = min(50,000, pre + nominal)` and `actual = post - pre`. Binding the complete snapshot and all derived fields prevents claim-versus-clear reordering from producing an interchangeable receipt.
...
- `segments` preserves chronological eligible elapsed by floor. Every entry has the exact key set `{floor, elapsedMs}`; `floor` is an integer `1…highestFloor`; `elapsedMs` is positive finite and at most `86,400,000`; entries are strictly increasing by floor after adjacent equal floors merge; length is at most 50; and the finite elapsed sum is at most `86,400,000`. Fractional milliseconds are preserved exactly because accepted predecessor/runtime clocks are finite rather than integer-only.
