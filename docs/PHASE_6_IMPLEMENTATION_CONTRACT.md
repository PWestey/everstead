...
companionTower: {
  ...
  idle: {
    cursorAt: non-negative safe-integer Tower tick,
    lastClaimAt: null | non-negative safe-integer Tower tick,
...
- Tower-clear identity binds save ID + floor + pre-clear sequence + the exact captured pre-clear claimed-interval history + every derived receipt field. A receipt records floor, normalized Tower time, sequence, requirement/total Power, identity, exact Companion EXP/shard maps, nominal Mastery entitlement, pre/post Mastery points, the actual cap-truncated Mastery award, and an exact `preClearClaimedIntervalsByFloor` snapshot with floor keys `1…50`.
- The pre-clear snapshot's safe-integer count sum is the historical idle interval ordinal at that clear. Expanded in canonical ascending-floor order, it must be an exact prefix of the current expanded claimed-interval history: all counts below an optional partial boundary equal current counts, the boundary count is at most current, and every higher count is zero. For a receipt clearing floor `F`, snapshot floors `F…50` are zero.
- Canonical replay of that snapshot plus nominal Tower-clear Mastery through floor `F - 1` derives the receipt's pre-Mastery points exactly. `post = min(50,000, pre + nominal)` and `actual = post - pre`. Binding the complete snapshot and all derived fields prevents claim-versus-clear reordering from producing an interchangeable receipt.
...
- Tower idle receipts record claim sequence/normalized Tower time, `preClaimClearSequence`, consumed elapsed, interval ordinal range, interval counts by floor, actual EXP/shard maps, nominal Mastery entitlement, pre/post Mastery points, actual cap-truncated Mastery award, pity after claim, identity version, and a pending-snapshot identity.
- The pending-snapshot identity binds `preClaimClearSequence` plus every pre-claim interval/segment/ordinal/pity field and every derived receipt field. Replay derives pre-Mastery from nominal clears through `preClaimClearSequence` plus cumulative idle intervals before the receipt's exact suffix. `post = min(50,000, pre + nominal)` and `actual = post - pre`. A stale preview cannot commit after another transaction, and later floor clears cannot make the historical claim snapshot ambiguous.
...
- Define `towerTick(timestamp) = min(Number.MAX_SAFE_INTEGER, max(0, floor(timestamp)))` for every finite timestamp entering Tower migration, preview, settlement, challenge, claim, receipt, retry, or validation. Normalize before maximums and subtraction; never subtract raw fractional or unsafe-magnitude clocks. Schema-7 Tower timestamps and elapsed values are safe integers.
- Use one normalized captured transaction tick. Extend central accrual so every persisted mutation settles eligible Tower elapsed under the old `highestFloor` before changing state.
- `segments` preserves chronological eligible elapsed by floor. Every entry has the exact key set `{floor, elapsedMs}`; `floor` is an integer `1…highestFloor`; `elapsedMs` is a positive safe integer at most `86,400,000`; entries are strictly increasing by floor after adjacent equal floors merge; length is at most 50; and the safe-integer elapsed sum is at most `86,400,000`.
...
- Initialize Mastery zero, Companion Campaign empty, Tower floor zero, no clears/claims/rewards, and Tower `cursorAt` at the exact maximum of `towerTick(captured migration time)` and `towerTick` of these four schema-6 fields only: `saveMeta.updatedAt`, `lastSeen`, `lastGoldAt`, and `familyDrops.eligibleAt`. No receipt or other catch-all timestamp participates.
...
- A lone evolved schema-6 pre-v7 checkpoint with both active and schema-7 staging missing remains intentionally fail-closed: it is not independently authenticatable. Retry requires the exact active predecessor or an authenticated staged schema-7 successor; only a deterministic immediate predecessor reconstruction is recoverable without either.
...
- Idle at raw `0/0.5/1 ms`, `Number.MAX_SAFE_INTEGER - 1 / exact / +1`, `Number.MAX_VALUE`, exact `towerTick` normalization for fractional/unsafe-magnitude migration and runtime clocks, interval `-1 / exact / +1`, multiple intervals, cap `-1 / exact / +1`, `>24h`, epoch zero, clock rollback, local midnight, DST boundaries, multiple floor changes, cross-floor partial interval, reload, byte-identical split/combined settlement, pity misses 1–7/forced eighth/reset, chance cap, recipient boundaries, stale preview, immediate double claim, and safe-integer overflow.
...
- Source ledger rejects internally consistent forged receipts/totals, altered current or pre-clear claimed interval counts, claim-versus-clear reorderings, derived pity/roll/reward/Mastery-snapshot mismatches, extra/missing reward keys, sequence rollback, foreign save/stage/floor identities, claimed-without-clear, skipped progression, replay-before-clear, and unexplained Companion EXP/shards/rarity/Mastery.
