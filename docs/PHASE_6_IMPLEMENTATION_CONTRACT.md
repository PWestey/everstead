...
- Define `towerTick(timestamp) = max(0, floor(timestamp))` for every finite timestamp entering Tower migration, preview, settlement, challenge, claim, receipt, retry, or validation. Normalize before maximums and subtraction; never subtract raw fractional clocks. Schema-7 Tower timestamps and elapsed values are safe integers.
