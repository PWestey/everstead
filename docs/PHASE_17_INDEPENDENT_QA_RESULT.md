...
- package-only static verifier: **55/55 passed**
- exact current-candidate static verifier: **54/64 passed, 10 failed**
- checksum manifest: **10/10 passed**
- accepted Phase 17 design validator: **31/31 passed** inside the package verifier

The ten expected candidate failures are Phase 17-only: trusted QA bridge, Book I definitions, story successor/ordering, facility unlock/anchor alias runtime, Chronicle/scene DOM, native story finalizer, tutorial/cast runtime, Phase 17 inherited-seam attestation, the separate `player.wayfarer` Campaign contract, and the non-tautological production reduced-motion contract. All package, design, frozen-base, syntax, and ownership assertions pass.

The live runner has twelve package rows plus two rows in each of five isolated realms. Because the current candidate lacks the Phase 17 bridge, each realm must fail only `bridge-present` and `phase17-contract-unavailable`, then stop before any destructive call. The precise expected preimplementation rendering is **12/22 passed, 10 failed**. This task had no discoverable in-app browser connection, so that count is not claimed as observed evidence. Root review must run the page after attaching a browser and must not promote the package based on the prediction alone.

Hardening added after root review: passive-original-four comparisons now occur within one fixture at the same frozen timestamp and explicitly exclude `day`, `boostDay`, `lastGoldAt`, and `lastSeen`; reduced-motion evidence uses a production-observable root attribute, real computed styles, and a static media-query guard rather than a test-owned `matchMedia` patch; and Campaign must expose `player.wayfarer` as a separate Player Character with the canonical RGB art reserved for title/profile and an approved transparent or silhouette/static Campaign treatment.
