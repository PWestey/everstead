...
- The receipt records mode/version/identity, claim sequence/time, `preClaimBestRunSequence`, exact preimage fields `preClaimHighestStage`, `preClaimCursorAt`, `preClaimSegments`, `preClaimIntervalOrdinal`, and `preClaimPityMisses`, consumed elapsed, half-open interval-ordinal range, exact interval counts by stage, nominal Might, pre/post Might points, actual Might awarded, exact Fellow shard map, pity before/after, and a pending-snapshot identity. Bind all preimage fields into the receipt identity and semantically validate them against the canonical claim-history suffix; sequence alone is not sufficient to recover historical stage/cursor/segment/pity provenance after a later best-run receipt overwrites current best-run context.
- The pending-snapshot identity binds the pre-claim best-run sequence, highest stage, cursor, segments, cumulative interval map, interval ordinal, pity, and every derived receipt field. A stale preview cannot commit after another claim, push, or mutation.
