...
A later external Chromium run of committed `a20909c` reported `453/454`; the sole failure was `scoped-undo-diverged-refuses-without-write`. Production correctly refused without writing, but the harness captured its raw/revision baseline before the persisted navigation back to Oaths. Commit `e74591c` moved that baseline immediately after navigation and before the completed-Oath click.

The corrected browser contract then passed `454/454` twice in external Chromium with zero console or page errors. An independent in-app Chromium review also passed `454/454` with zero warning or error console entries. These runs exercised the rendered offline modal, modal Undo, Patrol-divergence refusal, Oath deletion, recovery screens, navigation, rosters, and Building upgrade path.
