...
- Every current schema-5 save carrying the schema-4-to-5 receipt requires its exact pre-v5 checkpoint. A pending, authenticated schema-5 migration stage may reconstruct and write that checkpoint from the still-active exact schema-4 predecessor before committing; once that predecessor is gone, a missing pre-v5 checkpoint is refused. The expected collision ledger is reconstructed from the checkpoint and compared byte-for-byte in definition order; false, reordered, extra, or omitted ledgers fail safely.
