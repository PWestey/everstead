...
- The immutable scenario registry owns exact artifact/config/reference identities, fixed time/save data, ordinals/droughts/carries, released constants and salts, roster/relic/progression state, policies, safety limits, and all 144 matrix coordinates. A byte-order authority covers the complete registry, while the exported canonical-input validator independently freezes every variable subtree. The fixed static row recursively injects an unknown member into all 369 registry object locations and all 90 canonical-input object locations, then also checks changed/reordered authority values; every case must fail closed.
...
The package is implemented as an unsealed repaired candidate. Fast writer diagnostics confirm all 240 exact production-probe vectors, strict foreign-key rejection, the discriminating production-order Gold value, zero bad Gold ledgers across 144 bundles, zero bad Campaign no-replay proofs, zero duplicate-resource failures, and the complete progression output schema. The accepted 624-row verifier still has no bypass and retains the complete 203-file snapshot. Final focused runs will execute from a fully materialized temporary clone, followed by successor, inherited, checksum, live, and independent-review evidence on one exact tip. Pending fields are intentional and must not be interpreted as a pass.
