...
- Campaign simulation enforces the frozen no-replay rule. A cleared stage is never selectable or runnable, each paid/rewarded receipt is a first clear of the exact next contiguous stage, stage 10 advances to a null selection, and later claim attempts emit no receipt, spend, reward, or Rank EXP. Duplicate identity is semantic (`campaign:<lane>:<stage>:first-clear`) and excludes receipt-array position. The proof reconstructs each pre-run roster from immutable canonical starting state plus deterministic prior idle and Campaign progression, then independently derives total roster Power, rank and Power access, stage sequence, exact global per-claim `campaignOrder`, target, efficiency, cost, stable Gift roll, rewards, history, spend, and resource deltas without receipt authority. Its oracle has separate starting-state, EXP/Rank, Power, efficiency, RNG, reward, Family-recipient, and upgrade-cost implementations rather than calling the execution helpers.
...
The package is implemented as an unsealed repaired candidate. A post-freeze writer diagnostic completed 624/624 with the exact category totals, registry order, and no-write result. It confirms all 240 exact production-probe vectors, strict foreign/`undefined`/negative-zero rejection, Date and arithmetic boundary refusal, the discriminating production-order Gold value, independently reconstructed Campaign access/Power/cost/global order, structurally separate idle clock/progress reconstruction, and zero bad Gold, Campaign, idle-reward, duplicate, or lost-resource proofs across all 144 bundles. Twelve integrated proof corruptions are rejected, including a repriced cross-lane reorder, a forged execution Power snapshot with reconciled spend, forced execution Gift RNG, forced execution access, and four idle event/settlement faults. The accepted verifier still has no bypass and retains the complete 203-file snapshot. Official focused repeats will execute from a fully materialized temporary clone, followed by successor, inherited, checksum, live, and independent-review evidence on one exact tip. Pending fields are intentional and must not be interpreted as a pass.
