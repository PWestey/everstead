...
- The immutable scenario registry owns exact artifact/config/reference identities, fixed time/save data, ordinals/droughts/carries, released constants and salts, roster/relic/progression state, policies, safety limits, and all 144 matrix coordinates. Unknown, missing, reordered, or unsafe values fail closed.
- Gold conservation keeps production Float64 Building generation and pending-Gold carry, collects only whole pending Gold, records every ordered pending/Gold operation with exact before/delta/after values, and replays the three modeled sinks—Building upgrades, Fellow Campaign, and Companion Campaign—in the identical JavaScript operation order. No tolerance or milligold surrogate is used.
- Every bundle exposes canonical metadata; inputs; per-Building, per-segment, pending, claimed, and spent Gold; Building components, next cost, and time-to-afford; complete combat/economy Power ownership and independently derived no-double-count evidence; progression, Campaign, idle/pity/RNG/carry, pacing, and safety outputs.
...
The package is implemented as an unsealed candidate. A local focused diagnostic executed every Phase 10B-owned row successfully: 623/624 overall, with the sole deliberate diagnostic failure being the skipped 203-file predecessor snapshot because those Documents-worktree files were cloud placeholders. The accepted verifier has no bypass and retains the complete 203-file snapshot. Final 624-row runs will execute from a fully materialized temporary clone, followed by successor, inherited, checksum, live, and independent-review evidence on one exact tip. Pending fields are intentional and must not be interpreted as a pass.
