...
The package is implemented as an unsealed repaired candidate. A post-freeze writer diagnostic completed 624/624 with the exact category totals, registry order, and no-write result. It confirms all 240 exact production-probe vectors, strict foreign/`undefined`/negative-zero rejection, Date and arithmetic boundary refusal, the discriminating production-order Gold value, independently reconstructed Campaign access/Power/Gold/cost/global order, structurally separate idle clock/progress reconstruction, and zero bad Gold, Campaign, idle-reward, duplicate, or lost-resource proofs across all 144 bundles. Sixteen integrated proof corruptions are rejected, including a repriced cross-lane reorder, a forged execution Power snapshot with reconciled spend, forced execution Gift RNG/access, retained rewards with false gate fields, false Gold and sequence blocks, a wholly omitted Campaign loop, and four idle event/settlement faults. Missing eligible Campaign rewards now invalidate both the Campaign proof and the lost-resource result. The accepted verifier still has no bypass and retains the complete 203-file snapshot. Official focused repeats will execute from a fully materialized temporary clone, followed by successor, inherited, checksum, live, and independent-review evidence on one exact tip. Pending fields are intentional and must not be interpreted as a pass.
