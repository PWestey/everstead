...
- Campaign simulation enforces the frozen no-replay rule. A cleared stage is never selectable or runnable, each paid/rewarded receipt is a first clear of the exact next contiguous stage, stage 10 advances to a null selection, and later claim attempts emit no receipt, spend, reward, or Rank EXP. Duplicate identity is semantic (`campaign:<lane>:<stage>:first-clear`) and excludes receipt-array position. The proof reconstructs each pre-run roster from immutable canonical starting state plus deterministic prior idle and Campaign progression, then independently derives total roster Power, stage sequence, target, efficiency, cost, stable Gift roll, rewards, history, spend, and resource deltas without receipt authority.
- Idle reward proof uses the natural identities `tower:<floor>:<intervalOrdinal>`, `expedition:<stage>:<intervalOrdinal>`, and `family:<building>:<rollOrdinal>`. It reconstructs the exact claim clock, accepted segments, progress attribution, interval/roll counts and ordinals, pity, stable RNG, recipients, awards, carries, and ending histories from immutable starting state plus canonical horizon/cadence/cap definitions before comparing receipts. The frozen report stores compact claim/count/first/last/digest and resource-fold evidence; verifier-only adversarial audits prove that wrong Campaign target/Gift/sequence/Power/cost/spend and duplicate, missing, misdirected, or settlement-forged idle events fail.
...
The package is implemented as an unsealed repaired candidate. A post-freeze writer diagnostic completed 624/624 with the exact category totals, registry order, and no-write result. It confirms all 240 exact production-probe vectors, strict foreign/`undefined`/negative-zero rejection, Date and arithmetic boundary refusal, the discriminating production-order Gold value, immutable Campaign Power/cost reconstruction, canonical idle clock/progress reconstruction, and zero bad Gold, Campaign, idle-reward, duplicate, or lost-resource proofs across all 144 bundles. The accepted verifier still has no bypass and retains the complete 203-file snapshot. Official focused repeats will execute from a fully materialized temporary clone, followed by successor, inherited, checksum, live, and independent-review evidence on one exact tip. Pending fields are intentional and must not be interpreted as a pass.
